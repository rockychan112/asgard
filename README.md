...
三步：装 skill → 照 `examples/profile.sample.yaml` 写一份你的资料 → 给 agent 设一个每日任务（宿主没有定时功能，就每天说一句「asgard 今日简报」，一样能用）。
...
`brief`（折射单条新闻）、`eval`（跑考卷）、skill 每日简报（见上）能用；建资料暂时要手动编辑文件，交互式引导和长期记忆还没做。
