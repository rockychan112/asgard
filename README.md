...
**同一条新闻，只挑出跟你有关的利害，和你这周真能做的事；跟你无关，就直接说无关。**

Asgard 按你是谁、在做什么、在意什么，把一条新闻重写成只对你成立的那部分——会怎么影响你、你这周能做什么。跟你没关系时，它直说无关，不硬凑一个角度。

下面是同一条真实新闻发给四个不同的人：三个人各有各的利害和动作，第四个——跳过。

[![同一条高 stakes 新闻，折射给四个身份：三条给出各自的利害与行动，第四条诚实 SKIP](design/hero.png)](https://rockychan112.github.io/asgard/)

<p align="center"><sub>一条新闻（固定示例），人人拿到同一份中立事实；变的只是它对谁意味着什么。最后一个是诚实 SKIP。　·　<a href="https://rockychan112.github.io/asgard/">▶ 打开可交互 demo</a></sub></p>

`预研 / pre-alpha` — 探索中的开源作品，不是成品。能跑的是 `brief`（按人折射一条新闻）和 `eval`（下面那套测试）。
...
真正赢的是两件更朴素的事：它更可靠地说出"这条不是你的"（SKIP 纪律），每条判断都挂一条你能翻出来核对的依据（可审计）。不是"更懂你"的魔法——是更有纪律、更透明。
...
- ⬜ `init` — 交互式建 persona。
- ⬜ `feedback` + 长期记忆 — 越用越懂你（up / down / correct / …；演进只"建议 → 你确认"，绝不静默改你的身份）。
