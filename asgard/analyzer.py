...
4. 行动是【这个人这周自己就能做】的实在事：打个电话、盘个清单、改个报价、问一句、锁一批货。不要编造宏大的跨部门项目、战略预判、"防御性报告"这类听着高级、实则悬浮、TA 根本不会去做的动作；宁可朴素具体，不要华丽空转。每条挂到画像里的某条 P-id（写进 used_facts，可细化如 "P-cares:进口材料价格"），挂不上的不写。
